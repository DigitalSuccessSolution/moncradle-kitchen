                  ...
                  {recentOrders.map(o => (
                    <div key={o._id} className="grid grid-cols-12 gap-3 px-6 py-3.5 items-center hover:bg-slate-50/60 transition-colors">
                      ...
                      <div className="col-span-3 sm:col-span-3 flex justify-end">
                        <span className={`inline-block text-[11px] px-2.5 py-1 rounded-full font-medium capitalize ${o.status === 'ready' ? 'bg-emerald-100 text-emerald-700' :
                ...
                {activeBatches.length === 0 ? (
                  ...
                  activeBatches.slice(0, 3).map(b => (
                    <div key={b._id} className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex items-center justify-between">
                      ...
                      <span className={`text-[11px] font-medium px-2 py-1 rounded-md shrink-0 capitalize ${b.status === 'preparing' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'
                        }`}>
                        {b.status}
                      </span>
                    </div>
